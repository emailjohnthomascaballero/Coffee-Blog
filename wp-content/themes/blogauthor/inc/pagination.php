...
if ( ! function_exists( 'blogauthor_ajax_pagination' ) ) :
    /**
     ...
     */
    function blogauthor_ajax_pagination($type) {
        global $wp_query; 
        if ( $wp_query->max_num_pages > 1  ) {

            ?>
            <div class="blogauthor-load-posts-btn-wrapper"
            data-page="1" 
            data-max-pages="<?php echo esc_attr($wp_query->max_num_pages);?>" 
            data-load-type="<?php echo esc_attr($type);?>"
            >
                <a href="#" class="">
                    <span class="blogauthor-ajax-loader"></span>
                    <button class="load-btn"><?php _e('Load More', 'blogauthor') ?></button>
                </a>
            </div>
            <?php
        }
...
if ( ! function_exists( 'blogauthor_load_posts' ) ) :
    /**
     ...
     *
     */
    function blogauthor_load_posts() {

        check_ajax_referer( 'blogauthor-load-posts-nonce', 'nonce' );
        ...
        if($posts->have_posts()):
            ...
            $error = new WP_Error( '500', __('No More Posts','blogauthor') );
...
add_action( 'wp_ajax_blogauthor_load_posts', 'blogauthor_load_posts' );
add_action( 'wp_ajax_nopriv_blogauthor_load_posts', 'blogauthor_load_posts' );