 ...
 */
function blogauthor_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial(
			...
			array(
				...
				'render_callback' => 'blogauthor_customize_partial_blogname',
			)
		);
		$wp_customize->selective_refresh->add_partial(
			'blogdescription',
			array(
				...
				'render_callback' => 'blogauthor_customize_partial_blogdescription',
			)
		);
	}

	/*Get default values to set while building customizer elements*/
	$default_options = blogauthor_get_default_customizer_values();
	/* Header Background Color*/
	$wp_customize->add_setting(
	    'blogauthor_options[header_bg_color]',
	    array(
	...
	$wp_customize->add_control(
	    new WP_Customize_Color_Control(
	        ...
	        'blogauthor_options[header_bg_color]',
	        array(
	            'label' => __('Header Background Color', 'blogauthor'),
	            'section' => 'colors',
	            'type' => 'color',
        		'priority' => 1,
	...
	$wp_customize->add_section( 'pro__section', array(
		'title'       => '' . esc_html__( 'View PRO Version', 'blogauthor' ),
		'priority'    => 2,
		'description' => sprintf(
			...
			sprintf( '<a href="%1$s" target="_blank" class="button button-primary">%2$s</a>', esc_url( blogauthor_get_pro_link() ), esc_html__( 'View Blogauthor PRO', 'blogauthor' ) )
		),
	) );
	$wp_customize->add_setting( 'blogauthor_style_view_pro_desc', array(
		'default'           => '',
		'sanitize_callback' => '__return_true',
	) );
	$wp_customize->add_control( 'blogauthor_style_view_pro_desc', array(
		'section' => 'pro__section',
		'type'    => 'hidden',
	) );
}
add_action( 'customize_register', 'blogauthor_customize_register' );
 ...
 * @return void
 */
function blogauthor_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function blogauthor_customize_partial_blogdescription() {
 ...
 */
function blogauthor_customize_preview_js() {
    wp_enqueue_script( 'blogauthor-customizer', get_template_directory_uri() . '/assets/js/customizer.js', array( 'customize-preview' ), _S_VERSION, true );

}
add_action( 'customize_preview_init', 'blogauthor_customize_preview_js' );
 ...
 * @since 1.0.0
 */
function blogauthor_customizer_control_scripts(){
    wp_enqueue_style('blogauthor-customizer-css', get_template_directory_uri() . '/assets/css/customizer.css');
    wp_enqueue_script('blogauthor-customizer-controls', get_template_directory_uri() . '/assets/js/customizer-admin.js', array('jquery', 'jquery-ui-sortable', 'customize-controls') );
}
add_action('customize_controls_enqueue_scripts', 'blogauthor_customizer_control_scripts', 0);

/**
 * Generate a link to the Hive Lite info page.
 */
function blogauthor_get_pro_link() {
	return 'https://www.themeinwp.com/theme/blogauthor-pro/';
}